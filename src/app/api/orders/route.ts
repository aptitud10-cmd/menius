...
export async function POST(request: NextRequest) {
  try {
    ...
    const { allowed } = await checkRateLimitAsync(`order:${ip}`, {
      limit: 10,
      windowSec: 60,
    });
    if (!allowed) {
    ...
    if (!verifyOrderToken(body._ot, restaurant_id_raw)) {
      logger.warn("Invalid order token", {
        ip,
    ...
    const loyalty_account_id =
      typeof body.loyalty_account_id === "string" &&
      UUID_RE.test(body.loyalty_account_id)
    ...
    if (
      !restaurant_id ||
      (!String(restaurant_id).startsWith("demo") &&
        !UUID_RE.test(String(restaurant_id)))
    ) {
      return NextResponse.json(
    ...
    const parsed = publicOrderSchema.safeParse({
      ...
      items,
      tip_amount:
        body.tip_amount !== undefined ? Number(body.tip_amount) : undefined,
    });
    if (!parsed.success) {
    ...
    if (restaurantDbError) {
      logger.error("Restaurant DB query failed", {
        restaurant_id,
        error: restaurantDbError.message,
      });
    }
    if (!restaurant) {
      return NextResponse.json(
        {
          error: bodyEn ? "Restaurant not found" : "Restaurante no encontrado",
        },
        { status: 404 },
      );
    }
    ...
    if (
      orderTypesEnabled &&
      orderTypesEnabled.length > 0 &&
      !orderTypesEnabled.includes(order_type)
    ) {
      return NextResponse.json(
        {
          error: en
      ...
      return NextResponse.json(
        {
          error: en
            ...
            : "Se requiere el número de mesa para órdenes en mesa.",
        },
        { status: 400 },
      );
    }
    ...
    try {
      ...
      if (restaurant.commission_plan !== true) {
        const { data } = await supabase
          .from("subscriptions")
          .select(
            "status, trial_end, current_period_end, plan_id, past_due_since",
          )
      ...
      logger.warn(
        "Subscription check failed during order creation — proceeding without plan enforcement",
        { error: subErr },
      );
    ...
    const { data: dbModOptions } =
      modGroupIds.length > 0
        ? await supabase
            .from("modifier_options")
            .select("id, group_id, price_delta")
            .in("group_id", modGroupIds)
        : { data: [] };
    ...
    const productsRequiringVariant = new Set(
      (dbVariants ?? []).map((v) => v.product_id),
    );
    ...
    const firstVariantByProduct = new Map<
      string,
      { id: string; price_delta: number }
    >();
    for (const v of dbVariants ?? []) {
      if (!firstVariantByProduct.has(v.product_id))
        firstVariantByProduct.set(v.product_id, v);
    }
    ...
    const groupsWithOptions = new Set(
      (dbModOptions ?? []).map((o) => o.group_id),
    );
    ...
    for (const item of parsed.data.items) {
      ...
      for (const mod of item.modifiers ?? []) {
        const isLegacy =
          !mod.option_id || String(mod.option_id).startsWith("__legacy");
        if (isLegacy) {
          logger.warn(
            "legacy modifier — using $0 for security (migrate to modifier_options)",
            {
              restaurant_id,
          ...
          logger.error(
            "modifier option not found in DB — possible forged request, using $0",
            {
              restaurant_id,
              option_id: mod.option_id,
              product: item.product_id,
          ...
          captureError(new Error("modifier option not found in DB"), {
            restaurant_id,
            option_id: mod.option_id,
            product: item.product_id,
          });
      ...
      for (const grp of groups) {
        ...
        if (selected < grp.min_select) {
          return NextResponse.json(
            {
              error: `Selecciona al menos ${grp.min_select} opción(es) en "${grp.name}".`,
            },
            { status: 400 },
          );
        }
        if (grp.max_select > 0 && selected > grp.max_select) {
          return NextResponse.json(
            { error: `Máximo ${grp.max_select} opción(es) en "${grp.name}".` },
            { status: 400 },
          );
        }
      }
      ...
      if (
        item.unit_price !== 0 &&
        Math.abs(item.unit_price - expectedUnitPrice) > 0.02
      ) {
        logger.error(
          "Price mismatch (client vs server) — server value applied",
          {
            restaurant_id,
            product: item.product_id,
        ...
        captureError(
          new Error(
            "Price mismatch: client sent different price than server calculated",
          ),
          {
            restaurant_id,
            product: item.product_id,
    ...
    if (promo_code) {
      const { data: promo } = await supabase
        .from("promotions")
        .select(
          "discount_type, discount_value, min_order, expires_at, max_uses, current_uses, is_active",
        )
        .eq("restaurant_id", restaurant_id)
    ...
    if (wantsLoyalty) {
      ...
      if (loyaltyConfig?.enabled) {
        ...
        const phoneMatches =
          ...
          !accountPhone ||
          accountPhone === normalizedOrderPhone;
        ...
        if (
          ...
          loyaltyDiscountAmt = Math.min(
            Math.floor(pointsToRedeem * loyaltyConfig.peso_per_point * 100) /
              100,
    ...
    const { tipAmt, deliveryFeeAmt, totalDiscountAmt, taxAmt, total } =
      computeOrderTotals({
    ...
    if (orderError) {
      if (orderError.code === "23505" && idempotencyKey) {
        ...
        if (raceWinner) {
          return NextResponse.json(
            {
              order_id: raceWinner.id,
              order_number: raceWinner.order_number,
      ...
      if (promoIncremented && orderError.code !== "23505") {
        adminDb
          .rpc("decrement_promo_usage", {
            p_code: promo_code!.toUpperCase().trim(),
            p_restaurant_id: restaurant_id,
          })
          .then(({ error: e }) => {
            if (e)
              logger.error("Failed to rollback promo usage", {
                error: e.message,
              });
          });
      }
      return NextResponse.json(
        { error: "No se pudo crear la orden. Intenta de nuevo." },
        { status: 500 },
      );
    }
    ...
    if (itemsError) {
      logger.error("order_items insert failed — attempting rollback", {
        order_id: order.id,
      ...
      if (rollbackErr) {
        ...
        logger.error(
          "rollback delete failed — marking order cancelled to prevent ghost",
          {
            order_id: order.id,
        ...
        await adminDb
          ...
          .update({
            status: "cancelled",
            notes: "[auto-cancelled: items insert failed]",
          })
          .eq("id", order.id);
      }
      return NextResponse.json(
    ...
    if (validatedLoyaltyPoints > 0 && loyalty_account_id) {
      try {
        const { data: redemptionResult, error: rpcError } = await adminDb.rpc(
        ...
        if (rpcError) {
          ...
          logger.warn(
            "Loyalty redemption returned 0 — insufficient points or account not found",
            {
              account_id: loyalty_account_id,
    ...
    parsed.data.items.forEach((item, idx) => {
      ...
      for (const mod of item.modifiers ?? []) {
        ...
        const serverPriceDelta = isLegacyMod
          ? 0
          : (modOptionMap.get(oid)?.price_delta ?? 0);
        ...
        const safeGroupId =
          !gid || gid.startsWith("__legacy") || !modGroupIdSet.has(gid)
            ? null
            : gid;
    ...
    if (extrasToInsert.length > 0 || modifiersToInsert.length > 0) {
      ...
      if (extrasToInsert.length > 0)
        insertJobs.push(
          Promise.resolve(
            adminDb.from("order_item_extras").insert(extrasToInsert),
          ),
        );
      if (modifiersToInsert.length > 0)
        insertJobs.push(
          Promise.resolve(
            adminDb.from("order_item_modifiers").insert(modifiersToInsert),
          ),
        );
      ...
      if (detailErrors.length > 0) {
        ...
        if (rollbackErr) {
          ...
          await adminDb
            ...
            .update({
              status: "cancelled",
              notes: "[auto-cancelled: detail insert failed]",
            })
            .eq("id", order.id);
        ...
        return NextResponse.json(
          {
            error: en
              ? "Error saving order details. Please try again."
              : "Error guardando detalles del pedido. Intenta de nuevo.",
          },
          { status: 500 },
        );
      }
    ...
    supabase
      .rpc("upsert_customer_from_order", {
        p_restaurant_id: restaurant_id,
      ...
      .then(({ error: custErr }) => {
        if (custErr) {
          logger.error("upsert_customer failed", { error: custErr.message });
          return;
        }
          ...
          adminDb
            ...
            .then(({ data: cust }) => {
              if (cust && (cust.tags ?? []).includes("reactivation_sent")) {
                const newTags = (cust.tags as string[]).filter(
                  (t) => t !== "reactivation_sent",
                );
                adminDb
                  .from("customers")
                  .update({ tags: newTags })
    ...
    const isColombianCurrency =
      (restaurant.currency ?? "").toUpperCase() === "COP";
    ...
    if (parsed.data.payment_method === "online" && !isColombianCurrency) {
      try {
        ...
        const connectedAccount = restStripe?.stripe_onboarding_complete
          ? restStripe?.stripe_account_id
          : null;
        ...
        logger.error("stripe session creation failed — rolling back order", {
          order_id: order.id,
          error: errMsg,
        });
        ...
        const { error: rbErr } = await adminDb
          .from("orders")
          .delete()
          .eq("id", order.id);
        if (rbErr) {
          logger.error(
            "rollback delete failed after Stripe error — marking cancelled",
            {
              order_id: order.id,
          ...
          await adminDb
            ...
            .update({
              status: "cancelled",
              notes: "[auto-cancelled: stripe session creation failed]",
            })
            .eq("id", order.id);
        }
    ...
    try {
      ...
      const extraIds = parsed.data.items.flatMap((i) =>
        (i.extras ?? []).map((e) => e.extra_id),
      );
      ...
      const notifItems = parsed.data.items.map((i) => {
        const variantName = i.variant_id
          ? variantMap.get(i.variant_id)?.name
          : undefined;
      ...
      void notifyNewOrder({
        ...
        logger.error("notifyNewOrder failed", {
          error: err instanceof Error ? err.message : String(err),
        });
      });
    } catch (err) {
      logger.error("notifyNewOrder dispatch failed", {
        error: err instanceof Error ? err.message : String(err),
      });
    }
...
function buildFirstOrderWowEmail(
