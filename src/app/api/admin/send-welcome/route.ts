  ...
  try {
    ...
    if (slugs.length === 0) {
      return NextResponse.json(
        { error: "Se requiere al menos un slug" },
        { status: 400 },
      );
    }
    ...
    if (!restaurants || restaurants.length === 0) {
      return NextResponse.json(
        {
          error: "No se encontraron restaurantes con email configurado",
          slugs,
        },
        { status: 404 },
      );
    ...
    for (const r of restaurants) {
      ...
      logger.info("Welcome email", {
        slug: r.slug,
        email: maskEmail(r.notification_email),
        ok,
      });
    }
    ...
    logger.error("send-welcome error", {
...
function buildWelcomeEmail(
  ...
  const body2 = en
    ? `If you have any questions, need help updating your menu, or run into anything unexpected — just reply to this email. We're a real team and we respond fast.`
    : `Si tienes alguna pregunta, necesitas ayuda para actualizar tu menú, o notas algo que no funciona — simplemente responde este correo. Somos un equipo real y respondemos rápido.`;
