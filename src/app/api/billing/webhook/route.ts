      ...
      switch (event.type) {
        ...
        case "customer.subscription.updated": {
          ...
          const interval = priceId
            ? getIntervalByStripePrice(priceId)
            : "monthly";
          ...
          if (
            status === "trialing" &&
            sub.trial_end &&
            new Date(sub.trial_end * 1000) < new Date()
          ) {
          ...
          if (sub.trial_start) {
            updateData.trial_start = new Date(
          ...
          if (dbError) {
            ...
            captureError(new Error(dbError.message), {
          ...
          if (
            resolvedId &&
            ["pro", "business"].includes(plan?.id ?? "") &&
            ["active", "trialing"].includes(status) &&
            previousStatus !== status
          ) {
            try {
              const { seedStyleAnchors } =
                await import("@/lib/seed-style-anchors");
              seedStyleAnchors(supabase, resolvedId).catch(() => {});
            } catch {
              /* non-blocking — anchor seeding never blocks the webhook response */
            }
          }
            ...
            try {
              ...
              const { data: userData } =
                await adminClient.auth.admin.getUserById(
                  restaurantData.owner_user_id,
                );
              const ownerEmail = userData?.user?.email;
              const ownerName =
                userData?.user?.user_metadata?.full_name || restaurantData.name;
              ...
              if (ownerEmail) {
                ...
                const { sendEmail, buildTrialEndingEmail } =
                ...
                // Mark as sent so the daily cron (trial-ending-reminders) skips this restaurant
                await supabase
                  .from("subscriptions")
                  .update({
