...
export async function GET(req: NextRequest) {
  ...
  if (driverError) {
    logger.error('Error al resolver driver', { userId: user.id, message: driverError.message });
    captureError(driverError, { route: 'GET /api/driver/orders', userId: user.id });
  ...
  if (ordersError) {
    logger.error('Error al obtener órdenes del driver', { driverId: driver.id, message: ordersError.message });
    captureError(ordersError, { route: 'GET /api/driver/orders', userId: user.id });
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
