...
export async function fetchMenuData(slug: string): Promise<MenuData | null> {
  try {
    ...
    try {
      const subscription = subData as { status: string; trial_end?: string | null; current_period_end?: string | null } | null;
      const now = new Date();
    ...
    console.error('[menu-data] Unexpected error in fetchMenuData', { slug, error: String(err) });
    return null;
  }
}
