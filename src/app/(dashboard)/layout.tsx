...
export default async function DashboardLayout({
  ...
  const defaultLocale = (restaurant?.locale === "en" ? "en" : "es") as
    | "es"
    | "en";
        ...
        <div className="min-h-[100dvh] bg-gray-50 text-gray-900 flex overflow-x-hidden">
          ...
          <aside className="hidden md:flex flex-col w-[260px] bg-white border-r border-gray-200 sticky top-0 h-dvh">
            ...
            <div className="px-5 pt-5 pb-4 border-b border-gray-100">
              <Link
                href="/app"
                className="flex items-center gap-2.5 min-w-0 mb-2"
              >
                <div className="min-w-0">
                  <p className="text-sm font-bold tracking-tight text-gray-900 truncate">
                    MENIUS
                  </p>
          ...
          {/* Main area */}
          <div className="flex-1 flex flex-col min-w-0 overscroll-y-contain">
            {/* Mobile header */}
            <header className="md:hidden bg-white border-b border-gray-200 px-4 h-14 flex items-center justify-between sticky top-0 z-30">
              <div className="flex items-center gap-2 min-w-0">
                <Link href="/app" className="flex items-center gap-2 min-w-0">
                  <span className="text-sm font-bold tracking-tight text-gray-900 flex-shrink-0">
                    MENIUS
                  </span>
                </Link>
                <RestaurantSwitcher
            ...
            <main
              id="main-content"
              className="flex-1 p-4 pb-24 md:p-6 lg:p-8 max-w-6xl w-full mx-auto overflow-x-hidden"
            >
              <Breadcrumbs />
              {children}
            </main>
