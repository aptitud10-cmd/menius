...
export default function ApiKeysPage() {
  ...
  const loadKeys = useCallback(async () => {
    ...
    try {
      ...
      else setError(data.error ?? 'Error cargando keys');
    } catch {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { setError(data.error ?? 'Error creando key'); return; }
    ...
    <div className="max-w-2xl space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Acceso programático a tu restaurante para integraciones externas.
          </p>
        </div>
        <button
      ...
      {newRawKey && (
        <div className="bg-amber-50 border-2 border-amber-300 rounded-2xl p-5">
          <div className="flex items-start gap-3 mb-3">
            ...
            <div>
              <p className="text-sm font-bold text-amber-800">¡Copia tu API Key ahora!</p>
              <p className="text-xs text-amber-700 mt-0.5">No volverás a verla completa. Guárdala en un lugar seguro.</p>
            </div>
          </div>
      ...
      {showCreate && (
        <div className="bg-white rounded-2xl border border-gray-200 p-5">
          <h3 className="text-sm font-bold text-gray-900 mb-3">Nueva API Key</h3>
          <div className="flex gap-2">
            <input
              value={newKeyName}
              onChange={e => setNewKeyName(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && handleCreate()}
              placeholder="Ej. Integración POS, Webhook custom…"
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 gap-3 text-gray-400">
            ...
            <p className="text-sm">Sin API keys todavía</p>
          </div>
        ) : (
              ...
              <div key={k.id} className={cn('flex items-center gap-4 px-5 py-4', !k.is_active && 'opacity-50')}>
                ...
                <div className="text-right flex-shrink-0">
                  <p className="text-xs text-gray-400">
                    Creada {new Date(k.created_at).toLocaleDateString('es-MX')}
                  </p>
                  {k.last_used_at && (
      ...
      <div className="bg-gray-50 rounded-2xl border border-gray-200 p-5">
        <h3 className="text-sm font-bold text-gray-900 mb-3">¿Cómo usarla?</h3>
        <p className="text-xs text-gray-600 mb-3">Incluye el header en tus requests:</p>
