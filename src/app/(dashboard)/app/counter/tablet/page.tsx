...
export const metadata = {
  title: 'Counter Tablet — MENIUS',
};

export default async function CounterTabletPage() {
  const { supabase, restaurantId } = await getDashboardContext();

  const [{ data: restaurant }, { data: orders }] = await Promise.all([
    supabase
      .from('restaurants')
      .select('name, currency, slug, locale, latitude, longitude, tax_label, tax_included')
      .eq('id', restaurantId)
      .maybeSingle(),
    supabase
      .from('orders')
  ...
  return (
    <div className="fixed inset-0 bg-[#F2F2F2] overflow-hidden">
      <CounterView
