    ...
    <div className="space-y-6">
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm text-zinc-400 mb-1">Tipo de descuento*</label>
              <select
                value={discountType}
            ...
            <div>
              <label className="block text-sm text-zinc-400 mb-1">Pedido mínimo ($)</label>
              <input
                type="number"
                step="0.01"
                min="0"
            ...
            <div>
              <label className="block text-sm text-zinc-400 mb-1">Usos máximos</label>
              <input
                type="number"
                min="1"
                value={maxUses}
            ...
            <div>
              <label className="block text-sm text-zinc-400 mb-1">Fecha de expiración</label>
              <input
                type="datetime-local"
                value={expiresAt}
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              type="button"
              onClick={resetForm}
              className="px-4 py-2 bg-zinc-800 text-zinc-400 rounded-lg hover:bg-zinc-700 transition"
            >
              Cancelar
            </button>
          </div>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {promos.map(p => (
            <div key={p.id} className={`flex items-center justify-between bg-zinc-900 border rounded-xl p-4 transition ${p.is_active ? 'border-zinc-800' : 'border-zinc-800/50 opacity-60'}`}>
