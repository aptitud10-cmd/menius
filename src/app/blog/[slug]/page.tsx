...
export async function generateMetadata({
  ...
  return {
    // El <title> usa la version corta cuando existe (Google corta cerca de los
    // 60 chars). openGraph/twitter siguen con el titular completo: al compartir
    // el enlace no hay ese limite y el titulo largo comunica mejor.
...
function getUiText(locale: string) {
  if (locale === "en")
    return {
      ...
      ctaDesc:
        "Create your digital menu in minutes. 14-day free trial, no credit card.",
...
export default async function BlogPostPage({ params }: PageProps) {
  ...
  const jsonLd = {
    ...
    publisher: {
      "@type": "Organization",
      name: "MENIUS",
      logo: { "@type": "ImageObject", url: `${APP_URL}/icons/icon-512.svg` },
    },
    mainEntityOfPage: {
      "@type": "WebPage",
                ...
                <Link key={r.slug} href={`/blog/${r.slug}`} className="group">
                  <article className="h-full p-5 rounded-2xl card-premium hover:-translate-y-1 transition-all duration-300">
                    ...
                    <p className="text-xs text-gray-600">
                      {r.readTime} min · {formatDate(r.date, locale)}
                    </p>
                  </article>
                </Link>
              ))}
