...
export function PublicMenuClient({
  ...
  return (
    <div className="min-h-screen-safe bg-gray-50 overflow-x-hidden">
      ...
      {/* ── Modals ── */}
      {selectedProduct && (
        <ProductModal product={selectedProduct} onClose={() => setSelectedProduct(null)} fmtPrice={fmtPrice} t={t} />
      )}
      {showInfo && <InfoModal restaurant={restaurant} onClose={() => setShowInfo(false)} t={t} />}
      {isOpen && <CartDrawer restaurant={restaurant} tableName={tableName} fmtPrice={fmtPrice} t={t} />}
    </div>
  );
}
...
function ProductModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end lg:items-center justify-center">
      ...
      <div className="relative w-full max-w-lg lg:max-w-2xl bg-white rounded-t-3xl lg:rounded-3xl max-h-[90vh] overflow-y-auto animate-scale-in shadow-2xl">
        <button
          onClick={onClose}
        ...
        <div className="lg:flex">
          {/* Image */}
          {product.image_url && (
            <div className="relative w-full lg:w-1/2 aspect-[4/3] lg:aspect-auto lg:min-h-[400px] bg-gray-100 flex-shrink-0">
              <Image src={product.image_url} alt={product.name} fill sizes="(max-width: 1024px) 100vw, 50vw" className="object-cover lg:rounded-l-3xl" />
              {product.is_featured && (
                <span className="absolute top-4 left-4 inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-orange-500 text-white text-xs font-bold shadow-lg">
                  <Flame className="w-3 h-3" /> {t.popular}
                </span>
              )}
            </div>
          ...
          {/* Content */}
          <div className="p-5 lg:p-6 lg:flex-1 lg:flex lg:flex-col">
            <div className="lg:flex-1">
              <h2 className="text-xl lg:text-2xl font-bold text-gray-900">{product.name}</h2>
              {product.description && <p className="text-sm text-gray-500 mt-1.5 leading-relaxed">{product.description}</p>}
              <p className="text-xl font-bold text-brand-600 mt-2">{fmtPrice(Number(product.price))}</p>

              {/* Variants */}
              {variants.length > 0 && (
                <div className="mt-5">
                  <label className="text-sm font-semibold text-gray-700 mb-2.5 block">{t.variant}</label>
                  <div className="flex flex-wrap gap-2">
                    {variants.map((v) => (
                      ...
                      >
                        {v.name} {v.price_delta > 0 && <span className="text-brand-500">+{fmtPrice(Number(v.price_delta))}</span>}
                      </button>
                    ))}
                  </div>
                </div>
              ...
              {extras.length > 0 && (
                <div className="mt-5">
                  <label className="text-sm font-semibold text-gray-700 mb-2.5 block">{t.extras}</label>
                  <div className="space-y-2">
                    {extras.map((ex) => (
                      <button
                        ...
                        className={cn(
                          'w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl border-2 transition-all text-left',
                          selectedExtras.find((e) => e.id === ex.id)
                            ? 'border-brand-600 bg-brand-50'
                            : 'border-gray-200 hover:border-gray-300'
                        )}
                      >
                        <span className="text-sm">{ex.name}</span>
                        <span className="text-sm font-medium text-brand-600">+{fmtPrice(Number(ex.price))}</span>
                      </button>
                    ))}
                  </div>
                </div>
            ...
            <div className="flex items-center gap-4 mt-5 lg:mt-6">
              <div className="flex items-center gap-3 bg-gray-100 rounded-xl px-1 py-1">
                <button onClick={() => setQty(Math.max(1, qty - 1))} className="w-9 h-9 flex items-center justify-center rounded-lg hover:bg-white transition-colors">
                  <Minus className="w-4 h-4" />
                </button>
                <span className="w-6 text-center font-bold">{qty}</span>
                <button onClick={() => setQty(qty + 1)} className="w-9 h-9 flex items-center justify-center rounded-lg hover:bg-white transition-colors">
                  <Plus className="w-4 h-4" />
                </button>
              </div>
              <button
                onClick={handleAdd}
                className="flex-1 py-3 rounded-xl bg-brand-600 text-white font-semibold shadow-lg shadow-brand-600/25 hover:-translate-y-0.5 hover:shadow-xl transition-all"
              >
