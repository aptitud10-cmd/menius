...
export function PublicMenuClient({
  ...
  return (
    <div className="min-h-screen-safe bg-gray-50 overflow-x-hidden">
      ...
      <header className={cn(
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-12 sm:h-14">
            <h2 className="text-sm sm:text-base font-bold truncate mr-4 font-heading text-gray-900">
              {restaurant.name}
            </h2>
            <div className="flex items-center gap-1">
              <button
                onClick={() => setLocale(locale === 'es' ? 'en' : 'es')}
                className="px-2.5 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider bg-gray-100 text-gray-500 hover:bg-gray-200 transition-colors"
              ...
              >
                ...
                {totalItems() > 0 && (
                  <span className="absolute top-1 right-1 w-4 h-4 flex items-center justify-center rounded-full bg-brand-600 text-white text-[9px] font-bold ring-2 ring-white">
                    {totalItems()}
                  </span>
                )}
              </button>
            ...
            <div className="pb-3 animate-fade-in">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-10 py-2.5 rounded-xl bg-gray-100 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/30 placeholder-gray-400"
                  autoFocus
                />
                {searchQuery && (
                  <button
          ...
          {/* Category pills inside nav (mobile) */}
          <div className="lg:hidden -mx-4 px-4 pb-2.5 flex gap-2 overflow-x-auto scrollbar-hide scroll-touch">
            <button
              onClick={() => { setActiveCategory(null); setSearchQuery(''); }}
              className={cn(
                'flex-shrink-0 px-4 py-2 rounded-full text-[13px] font-semibold transition-all whitespace-nowrap',
                activeCategory === null
      ...
      <div className={cn('max-w-7xl mx-auto px-4 sm:px-6 py-6', totalItems() > 0 && 'pb-28')}>
          ...
          <main className="flex-1 min-w-0">
            ...
            {filteredItems !== null ? (
              ...
              <div className="text-center py-20">
                ...
                <p className="text-sm text-gray-400">{locale === 'es' ? 'Pronto habrá productos disponibles' : 'Products coming soon'}</p>
              </div>
            ) : (
              <>
                    ...
                    >
                      <h2
                        className={cn(
                          ...
                          isDemo ? 'top-[calc(41px+5rem)] lg:top-[calc(41px+4rem)]' : 'top-[5rem] lg:top-[4rem]'
                        )}
                      >
                        {category.name}
      ...
      {totalItems() > 0 && (
        <div className="fixed bottom-0 left-0 right-0 z-30 pointer-events-none pb-[env(safe-area-inset-bottom)]">
          <div className="p-4 pt-10 bg-gradient-to-t from-white via-white/95 to-transparent">
            <div className="max-w-lg mx-auto pointer-events-auto">
              <button
                onClick={() => setOpen(true)}
                className="w-full flex items-center justify-between px-5 py-4 rounded-2xl bg-brand-600 text-white shadow-[0_8px_30px_rgba(0,0,0,0.15)] hover:bg-brand-700 active:scale-[0.98] transition-all"
              >
                <div className="flex items-center gap-3">
                  <div className="relative">
                    <ShoppingBag className="w-5 h-5" />
                    <span className="absolute -top-1.5 -right-1.5 w-4 h-4 flex items-center justify-center rounded-full bg-white text-brand-700 text-[10px] font-bold">
                      {totalItems()}
                    </span>
                  </div>
                  <span className="font-semibold text-[15px]">{t.viewCart}</span>
                </div>
                <span className="font-bold text-[15px] tabular-nums">{fmtPrice(totalPrice())}</span>
              </button>
            </div>
          </div>
        </div>
...
function CartDrawer({
  ...
  const enabledOrderTypes: string[] = restaurant.order_types_enabled ?? ['dine_in', 'pickup'];
  const enabledPaymentMethods: string[] = restaurant.payment_methods_enabled ?? ['cash'];
  ...
  const closeDrawer = () => { setOpen(false); setOrderNumber(null); setOrderId(null); };
  ...
  if (orderNumber) {
    return (
      <div className="fixed inset-0 z-50">
        ...
        <div className="absolute inset-0 sm:inset-y-0 sm:right-0 sm:left-auto sm:w-[480px] bg-white flex flex-col animate-[slideUp_0.35s_cubic-bezier(0.32,0.72,0,1)] sm:animate-[slideInRight_0.3s_ease-out] shadow-2xl">
          ...
          <div className="flex-1 flex flex-col items-center justify-center px-8">
            <div className="w-20 h-20 rounded-full bg-gradient-to-br from-emerald-100 to-emerald-50 flex items-center justify-center mb-6 animate-[scaleIn_0.4s_ease-out]">
              <svg className="w-9 h-9 text-emerald-600" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" />
              </svg>
            </div>
            <h2 className="text-2xl font-bold mb-2 font-heading text-center">{t.orderSent}</h2>
            <p className="text-gray-500 text-sm mb-6 text-center max-w-xs leading-relaxed">{t.orderSentDesc}</p>
            <div className="px-5 py-3 bg-gray-50 rounded-2xl border border-gray-100 mb-8">
              <p className="text-xs text-gray-400 text-center mb-1">{locale === 'es' ? 'Número de orden' : 'Order number'}</p>
              <p className="font-mono font-bold text-xl text-center tracking-wider">{orderNumber}</p>
            </div>
            <div className="flex flex-col gap-3 w-full max-w-xs">
              {orderId && (
                <button
                  ...
                  disabled={payLoading}
                  className="w-full py-3.5 rounded-2xl bg-emerald-600 text-white font-semibold hover:bg-emerald-700 disabled:opacity-50 transition-colors shadow-lg shadow-emerald-600/20"
                >
              ...
              <a
                ...
                className="w-full py-3.5 rounded-2xl bg-brand-600 text-white font-semibold hover:bg-brand-700 text-center transition-colors shadow-lg shadow-brand-600/20"
              >
                {t.trackOrder}
              </a>
              <button
                onClick={closeDrawer}
                className="w-full py-3 rounded-2xl text-gray-500 font-medium text-sm hover:bg-gray-50 transition-colors"
              >
                {t.backToMenu}
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto overscroll-contain">
              {items.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-gray-400 px-8">
                  ...
                  <p className="font-semibold text-gray-600 mb-1">{t.cartEmpty}</p>
                  <p className="text-sm text-center">{t.cartEmptyDesc}</p>
                  <button
                    onClick={() => setOpen(false)}
                    className="mt-6 px-6 py-2.5 rounded-xl bg-brand-600 text-white text-sm font-semibold hover:bg-brand-700 transition-colors"
                  >
                    {t.backToMenu}
                  </button>
                </div>
                ...
                <div className="px-5 py-4 space-y-3">
                  {items.map((item, idx) => (
                    <div key={idx} className="flex gap-3 p-3.5 rounded-2xl bg-gray-50/80 border border-gray-100">
                      {item.product.image_url ? (
                        <div className="relative w-16 h-16 rounded-xl overflow-hidden bg-gray-200 flex-shrink-0 ring-1 ring-black/5">
                        ...
                        <div className="flex items-start justify-between gap-2">
                          <div className="min-w-0">
                            ...
                            {item.extras.length > 0 && (
                              <p className="text-xs text-gray-400 mt-0.5 truncate">+{item.extras.map((e) => e.name).join(', ')}</p>
                            )}
                            {item.notes && <p className="text-xs text-gray-400 mt-0.5 italic truncate">&quot;{item.notes}&quot;</p>}
                          </div>
                          <button onClick={() => removeItem(idx)} className="p-1 rounded-lg text-gray-300 hover:text-red-500 hover:bg-red-50 transition-colors flex-shrink-0">
                        ...
                        <div className="flex items-center justify-between mt-2">
                          <div className="flex items-center bg-white rounded-xl border border-gray-200 shadow-sm">
                            <button
                              ...
                              className="w-8 h-8 flex items-center justify-center rounded-l-xl hover:bg-gray-50 active:bg-gray-100 transition-colors"
                            >
                            ...
                            <button
                              onClick={() => updateQty(idx, item.qty + 1)}
                              className="w-8 h-8 flex items-center justify-center rounded-r-xl hover:bg-gray-50 active:bg-gray-100 transition-colors"
                            >
                              <Plus className="w-3 h-3 text-gray-500" />
                            </button>
                          </div>
                          <span className="text-sm font-bold text-gray-900">{fmtPrice(item.lineTotal)}</span>
                        </div>
                      </div>
                    </div>
                  ))}
            ...
            {items.length > 0 && (
              <div className="border-t border-gray-100 px-5 py-4 space-y-3 flex-shrink-0 bg-white">
                <div className="flex justify-between items-baseline">
                  ...
                  <span className="text-2xl font-bold text-gray-900">{fmtPrice(totalPrice())}</span>
                </div>
                <button
                  onClick={() => setShowCheckout(true)}
                  className="w-full py-3.5 rounded-2xl bg-brand-600 text-white font-semibold text-[15px] shadow-lg shadow-brand-600/25 hover:bg-brand-700 active:scale-[0.98] transition-all"
                >
                  {t.sendOrder}
                </button>
              </div>
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto overscroll-contain px-5 py-5 space-y-5">
              ...
              {enabledOrderTypes.length > 1 && (
                <div>
                  <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2.5">{t.orderType}</label>
                  <div className="grid grid-cols-3 gap-2">
                    {enabledOrderTypes.map((type) => (
                      <button
                        ...
                        className={cn(
                          'py-3 px-2 rounded-2xl text-sm font-semibold text-center transition-all border-2',
                          orderType === type
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">{t.yourName}</label>
                  <input
                    type="text"
                    value={customerName}
                    onChange={(e) => setCustomerName(e.target.value)}
                    placeholder={t.yourNamePlaceholder}
                    className="w-full px-4 py-3 rounded-2xl border-2 border-gray-200 text-sm focus:outline-none focus:border-brand-400 transition-colors"
                    autoFocus
                  />
                </div>
                <div>
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">
                    {t.orderNotes} <span className="text-gray-300 font-normal normal-case">({locale === 'es' ? 'opcional' : 'optional'})</span>
                  </label>
                  <textarea
                    value={orderNotes}
              ...
              {enabledPaymentMethods.length > 1 && (
                <div>
                  <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2.5">{t.paymentMethod}</label>
              ...
              {/* Promo code */}
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">{t.promoCode}</label>
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-4 py-3 rounded-2xl border-2 border-gray-200 text-sm focus:outline-none focus:border-brand-400 transition-colors uppercase"
                  />
                  <button
                    type="button"
                    onClick={validatePromo}
                    disabled={promoLoading || !promoCode.trim()}
                    className="px-5 py-3 rounded-2xl bg-gray-100 text-sm font-semibold hover:bg-gray-200 disabled:opacity-50 transition-colors"
                  >
              ...
              <div className="bg-gray-50 rounded-2xl p-4 space-y-2.5 border border-gray-100">
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">{locale === 'es' ? 'Resumen' : 'Summary'}</p>
                {items.map((item, idx) => (
                  <div key={idx} className="flex justify-between text-sm">
                    <span className="text-gray-600 truncate mr-2">{item.qty}x {item.product.name}</span>
                    <span className="font-medium text-gray-800 flex-shrink-0">{fmtPrice(item.lineTotal)}</span>
                  </div>
                ))}
                {discount > 0 && (
            ...
            {/* Checkout Footer */}
            <div className="border-t border-gray-100 px-5 py-4 space-y-2.5 flex-shrink-0 bg-white">
              {orderError && (
                <div className="flex items-center gap-2 px-3.5 py-2.5 rounded-xl bg-red-50 border border-red-100 animate-fade-in">
              ...
              <button
                ...
                disabled={submitting}
                className="w-full py-3.5 rounded-2xl bg-brand-600 text-white font-semibold text-[15px] shadow-lg shadow-brand-600/25 hover:bg-brand-700 active:scale-[0.98] transition-all disabled:opacity-50"
              >
                {submitting ? (
