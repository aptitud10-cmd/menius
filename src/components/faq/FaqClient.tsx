  ...
  return (
    <>
      ...
      <section className="sticky top-16 z-40 bg-[#050505]/80 backdrop-blur-2xl border-b border-white/[0.04]">
        {/* min-w-0: allows horizontal scroll inside overflow-x-clip ancestors (html/body) */}
        <div className="max-w-5xl mx-auto px-4 sm:px-6 relative min-w-0">
          <div className="relative min-w-0 pr-10 sm:pr-12">
            <nav
              className="faq-category-tabs flex flex-nowrap items-center gap-1 sm:gap-2 py-3 w-full min-w-0"
            ...
            >
              {categories.map((cat) => (
                <button
                  ...
                  className={cn(
                    'flex-shrink-0 inline-flex items-center gap-1.5 px-3 sm:px-4 py-2 rounded-lg text-sm font-medium transition-colors duration-200 whitespace-nowrap',
                    activeId === cat.id
                ...
                >
                  ...
                  <span>{cat.title}</span>
                  {activeId === cat.id && (
                    <span className="ml-0.5 flex-shrink-0 px-1.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-400 text-[10px] font-bold">
            ...
            {/* Fade hint — does not block touch scroll (pointer-events-none); pr-* on wrapper leaves room to scroll last tab fully in view */}
            <div
              className="pointer-events-none absolute right-0 top-0 bottom-0 w-10 sm:w-14 bg-gradient-to-l from-[#050505]/95 via-[#050505]/70 to-transparent"
              aria-hidden
            />
          </div>
        </div>
