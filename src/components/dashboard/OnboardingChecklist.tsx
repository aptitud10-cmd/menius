  ...
  useEffect(() => {
    ...
    try {
      ...
      if (printerRaw) {
        ...
        setHasPrinterConfigured(
          pCfg.receiptEnabled === true || pCfg.kitchenEnabled === true,
        );
      }
    } catch {
      /* ignore */
    ...
    const counterVisited =
      localStorage.getItem("menius-counter-visited") === "true";
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "p-5 pb-4",
          isNewUser && "bg-gradient-to-br from-violet-50 to-white",
        )}
      >
        {isNewUser && (
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <Sparkles
                className={cn(
                  "w-4.5 h-4.5",
                  isNewUser ? "text-violet-600" : "text-emerald-600",
                )}
              />
            </div>
            <div>
        ...
        {/* Preview CTA — visible once the owner has at least 1 product */}
        {steps.hasProducts && (
          <a
            href={`/${restaurantSlug}`}
            target="_blank"
            rel="noopener noreferrer"
            className="mt-3 flex items-center justify-center gap-2 w-full py-2.5 rounded-xl text-sm font-semibold transition-colors border border-gray-200 bg-white text-gray-700 hover:bg-gray-50 hover:border-gray-300"
          >
            <Eye className="w-4 h-4" />
            {t.onboarding_previewMenu}
          </a>
        )}
      </div>
      ...
      <div className="border-t border-gray-200 bg-gray-50/50">
        ...
        {optionalSteps.map((step, i) =>
          renderStep(step, i, optionalSteps, true),
        )}
      </div>
    </div>
  );
