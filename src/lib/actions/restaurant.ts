...
export async function assignDriver(
  ...
  // Reuse the pre-generated token from order creation if it exists —
  // that's what's on the printed receipt QR. Don't invalidate it on assign.
  ...
  // Only reset GPS when assigning a NEW driver (new token issued)
  if (!existingToken) {
