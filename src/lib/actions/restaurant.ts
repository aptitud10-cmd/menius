...
export async function createVariant(
  productId: string,
  data: { name: string; price_delta: number; sort_order: number },
) {
  const {
    supabase,
    restaurantId,
...
export async function createExtra(
  productId: string,
  data: { name: string; price: number; sort_order: number },
) {
  const {
    supabase,
    restaurantId,
...
/**
 * Turns a product's legacy `product_variants` / `product_extras` into modifier
 * groups, then deletes the originals.
 *
 * The legacy tables predate modifier_groups and cannot express a required
 * choice, a default option, or a conditional group, so the editor only ever
 * offered "delete" for them — which threw the data away. This migrates instead.
 *
 * Variants become one required single-select group (they were mutually
 * exclusive sizes/styles, and the first one was the implicit default).
 * Extras become one optional multi-select group (they were independent add-ons).
 *
 * Not a transaction: Supabase JS has no multi-statement transaction. The order
 * below is deliberate — groups and options are written first, and the legacy
 * rows are deleted only once that succeeded. A failure halfway leaves the
 * legacy rows intact and the new group visible, which is recoverable by hand;
 * the reverse order would lose the data outright.
 */
export async function migrateLegacyOptions(productId: string) {
  const {
    supabase,
    restaurantId,
    restaurantSlug,
  ...
  if (variantsRes.error || extrasRes.error || groupsRes.error) {
    const err = variantsRes.error || extrasRes.error || groupsRes.error;
  ...
  if (variants.length === 0 && extras.length === 0) {
    return { error: "Nada que migrar" };
  }

  // Append after whatever groups the product already has.
  let nextSort = Math.max(
    0,
    ...(groupsRes.data ?? []).map((g) => (g.sort_order ?? 0) + 1),
  );

  const created: string[] = [];
  const migrated = { variants: 0, extras: 0 };
  ...
  if (variants.length > 0) {
    const { data: group, error } = await supabase
      ...
      .select("id")
      .single();
    if (error || !group) return { error: error?.message ?? "Error" };
    created.push(group.id);

    const { error: optErr } = await supabase.from("modifier_options").insert(
      variants.map((v, i) => ({
        group_id: group.id,
        name: sanitizeText(v.name, 100),
        price_delta: v.price_delta ?? 0,
        // A required single-select needs a default, or the customer is forced
        // to choose before they can add anything to the cart.
        is_default: i === 0,
        sort_order: i,
      })),
    );
  ...
  if (extras.length > 0) {
    ...
    const { error: optErr } = await supabase.from("modifier_options").insert(
      extras.map((e, i) => ({
        ...
        price_delta: e.price ?? 0,
        is_default: false,
        sort_order: i,
      })),
    );
    if (optErr) return { error: optErr.message };
    migrated.extras = extras.length;
  }

  // Only now that the new groups exist do we drop the originals.
  const [delVariants, delExtras] = await Promise.all([
    variants.length > 0
      ? supabase.from("product_variants").delete().eq("product_id", productId)
      : Promise.resolve({ error: null }),
    extras.length > 0
      ? supabase.from("product_extras").delete().eq("product_id", productId)
      : Promise.resolve({ error: null }),
  ]);
  if (delVariants.error || delExtras.error) {
    // The groups are already live, so the customer sees the right options —
    // the leftover legacy rows just mean the amber panel stays visible.
    const err = delVariants.error || delExtras.error;
    return { error: `Migrado, pero no se pudo limpiar: ${err?.message}` };
  }

  revalidatePath("/app/menu/products");
  revalidatePublicMenu(restaurantSlug);
  return { success: true, created, migrated };
}
