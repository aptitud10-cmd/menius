...
export async function earnLoyaltyPoints({
  ...
  try {
    ...
    if (customerPhone) {
      ...
      if (acct) {
        accountId = acct.id;
        currentPoints = acct.points;
        currentLifetime = acct.lifetime_points;
      }
    }
    ...
    // Record transaction — UNIQUE (restaurant_id, order_id) prevents double-earn on concurrent calls
    const { error: txErr } = await adminDb.from("loyalty_transactions").insert({
      restaurant_id: restaurantId,
    ...
    if (txErr) {
      // 23505 = unique_violation: already credited by a concurrent call
      if (txErr.code === "23505") return;
      throw txErr;
    }
    ...
    if (customerEmail) {
      ...
      const html = buildLoyaltyEarnEmail({
        customerName,
        restaurantName,
        earnedPoints,
        newBalance,
